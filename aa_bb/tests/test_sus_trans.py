...
# AA BigBrother
from aa_bb.checks.sus_trans import get_user_transactions, is_transaction_hostile
from aa_bb.checks_cb.sus_trans import get_user_transactions as get_user_transactions_cb
...
class TestSusTrans(TestCase):
    ...
    def test_major_hub_filtering_enabled(self):
        ...
        self.assertFalse(
            is_transaction_hostile(tx), "Transaction in major hub should be filtered out when major hubs are disabled"
        )
    ...
    def test_major_hub_hostile_corp_show_market_enabled(self):
        ...
        self.assertFalse(
            is_transaction_hostile(tx),
            "Hostile market transaction in major hub should be filtered out when major hubs are disabled and show_market_transactions is True",
        )
    ...
    def test_major_hub_hostile_corp_show_market_disabled(self):
        ...
        self.assertFalse(
            is_transaction_hostile(tx),
            "Hostile market transaction in major hub should be filtered out even when show_market_transactions is False if hubs are disabled",
        )
    ...
    def test_string_system_id(self):
        ...
        self.assertFalse(
            is_transaction_hostile(tx), "String system_id should still be filtered out by major hub filter"
        )
    ...
    def test_major_hub_hostile_corp_show_market_disabled_cb(self):
        ...
        self.assertFalse(
            is_transaction_hostile_cb(tx),
            "Hostile market transaction in major hub should be filtered out (corp version)",
        )
    ...
    def test_string_system_id_cb(self):
        ...
        self.assertFalse(
            is_transaction_hostile_cb(tx),
            "String system_id should still be filtered out by major hub filter (corp version)",
        )
    ...
    def test_transaction_location_hostility(self, mock_is_hostile):
        ...
        self.assertTrue(
            is_transaction_hostile(tx), "Transaction in hostile location should be hostile if it's a suspicious type"
        )
        ...
        self.assertFalse(
            is_transaction_hostile(tx),
            "Normal transaction in hostile location should NOT be hostile if parties are safe/neutral",
        )
    ...
    def test_structure_id_resolution_cb(
        self, mock_get_type, mock_get_entity_info, mock_resolve_system, mock_resolve_name
    ):
        # Setup mock for get_entity_info
        mock_get_entity_info.return_value = {
            "name": "Test Character",
