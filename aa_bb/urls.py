...
urlpatterns = [
    ...
    path(
        "manual/settings/", views_faq.manual_settings, name="manual_settings"
    ),  # Manual section listing module settings.
    path(
    ...
    path(
        "stream_transactions_sse/", views.stream_transactions_sse, name="stream_transactions_sse"
    ),  # SSE feed for wallet transactions.
    ...
    path(
        "list_contract_ids/", views.list_contract_ids, name="list_contract_ids"
    ),  # Provide IDs for contract pagination.
    path(
        "check_contract_batch/", views.check_contract_batch, name="check_contract_batch"
    ),  # Fetch specific contract batch details.
    # Blacklist management
    path(
        "blacklist/add/", views.add_blacklist_view, name="add_blacklist"
    ),  # Simple UI to add entries to corp blacklist.
