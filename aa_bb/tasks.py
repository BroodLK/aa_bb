...
@shared_task
def cleanup_entity_cache():
    """
    Cleanup old EntityInfoCache entries to prevent unbounded memory growth.
    Keeps entries from the last 90 days and limits total entries to 50k.
    """
    from datetime import timedelta
    from .models import EntityInfoCache

    logger.info("✅  [AA-BB] - [cleanup_entity_cache] - Starting EntityInfoCache cleanup")

    # Remove entries older than 90 days
    cutoff = timezone.now() - timedelta(days=90)
    deleted_old, _ = EntityInfoCache.objects.filter(updated__lt=cutoff).delete()
    logger.info(f"✅  [AA-BB] - [cleanup_entity_cache] - Deleted {deleted_old} entries older than 90 days")

    # If still too many entries, remove oldest until we're under limit
    total_count = EntityInfoCache.objects.count()
    if total_count > 50000:
        excess = total_count - 40000  # Remove down to 40k to give some buffer
        old_ids = list(EntityInfoCache.objects.order_by('updated').values_list('id', flat=True)[:excess])
        EntityInfoCache.objects.filter(id__in=old_ids).delete()
        logger.info(f"✅  [AA-BB] - [cleanup_entity_cache] - Deleted {len(old_ids)} oldest entries to stay under limit")

    logger.info("✅  [AA-BB] - [cleanup_entity_cache] - Cleanup complete")


@shared_task
def cleanup_character_caches():
    """
    Cleanup old character-related cache entries based on last_accessed timestamps.
    Prevents memory leaks from accumulated cache data.
    """
    from datetime import timedelta
    from .models import (
    ...
    logger.info("✅  [AA-BB] - [cleanup_character_caches] - Starting character cache cleanup")

    # Remove entries not accessed in 30 days
    cutoff = timezone.now() - timedelta(days=30)

    deleted_emp, _ = CharacterEmploymentCache.objects.filter(last_accessed__lt=cutoff).delete()
    logger.info(f"✅  [AA-BB] - [cleanup_character_caches] - Deleted {deleted_emp} CharacterEmploymentCache entries")

    deleted_freq, _ = FrequentCorpChangesCache.objects.filter(last_accessed__lt=cutoff).delete()
    logger.info(f"✅  [AA-BB] - [cleanup_character_caches] - Deleted {deleted_freq} FrequentCorpChangesCache entries")

    deleted_stint, _ = CurrentStintCache.objects.filter(last_accessed__lt=cutoff).delete()
    logger.info(f"✅  [AA-BB] - [cleanup_character_caches] - Deleted {deleted_stint} CurrentStintCache entries")

    # AwoxKillsCache gets longer TTL (60 days)
    awox_cutoff = timezone.now() - timedelta(days=60)
    deleted_awox, _ = AwoxKillsCache.objects.filter(last_accessed__lt=awox_cutoff).delete()
    logger.info(f"✅  [AA-BB] - [cleanup_character_caches] - Deleted {deleted_awox} AwoxKillsCache entries")

    logger.info("✅  [AA-BB] - [cleanup_character_caches] - Cleanup complete")


@shared_task
def cleanup_processed_records():
    """
    Archive/cleanup old ProcessedContract, ProcessedMail, ProcessedTransaction records.
    Keeps last 180 days to balance memory usage with avoiding re-processing.
    """
    from datetime import timedelta
    from .models import ProcessedContract, ProcessedMail, ProcessedTransaction

    logger.info("✅  [AA-BB] - [cleanup_processed_records] - Starting processed records cleanup")

    # Remove records older than 180 days
    cutoff = timezone.now() - timedelta(days=180)

    deleted_contracts, _ = ProcessedContract.objects.filter(processed_at__lt=cutoff).delete()
    logger.info(f"✅  [AA-BB] - [cleanup_processed_records] - Deleted {deleted_contracts} ProcessedContract entries")

    deleted_mails, _ = ProcessedMail.objects.filter(processed_at__lt=cutoff).delete()
    logger.info(f"✅  [AA-BB] - [cleanup_processed_records] - Deleted {deleted_mails} ProcessedMail entries")

    deleted_trans, _ = ProcessedTransaction.objects.filter(processed_at__lt=cutoff).delete()
    logger.info(f"✅  [AA-BB] - [cleanup_processed_records] - Deleted {deleted_trans} ProcessedTransaction entries")

    logger.info("✅  [AA-BB] - [cleanup_processed_records] - Cleanup complete")
