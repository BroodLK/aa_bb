"""Helpers for periodic task naming and scheduler synchronization."""
    ...
    if updated:
        try:
            with transaction.atomic():
                task.save()
            logger.info(
                f"✅  [AA-BB] - [Tasks] - {'Created' if task.pk is None else 'Updated'} '{standardized_name}' periodic task (enabled={enabled})"
            )
...
def sync_periodic_tasks():
    ...
    for task_info in tasks_to_sync:
        ...
        logger.debug(
            f"ℹ️  [AA-BB] - [Tasks] - Task '{name}': feature_enabled={feature_enabled}, master_active={master_active} -> enabled={enabled}"
        )

        if existing_task and existing_task.enabled and not enabled:
            if not master_active:
                logger.warning(
                    f"ℹ️  [AA-BB] - [Tasks] - '{name}' will be DISABLED because Big Brother is globally inactive."
                )
            elif not feature_enabled:
                logger.warning(
                    f"ℹ️  [AA-BB] - [Tasks] - '{name}' will be DISABLED because its feature toggle '{task_info['active_attr']}' is False."
                )
